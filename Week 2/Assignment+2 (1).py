...
####Linear Score R_2
    ...
    X_poly = poly.fit_transform(X_train.reshape(11,1)) 
    linreg = LinearRegression().fit(X_poly, y_train)
    r2_train = linreg.score(X_poly, y_train)
    X_test_poly = poly.fit_transform(X_test.reshape(4,1))
    LinearRegression_R2_test_score =r2_score(y_test,linreg.predict(X_test_poly))
####Lasso Score R_2  

    linreg_lasso = Lasso(alpha=0.01, max_iter = 10000).fit(X_poly, y_train)
    
    r2_train_lasso = linreg_lasso.score(X_poly, y_train)
...
def answer_five():
    ...
    #for features_name, features_value in zip(X_train2.columns,clf.feature_importances_):
    
    features = zip(X_train2.columns,clf.feature_importances_)
    ...
    #top_5 = sorted_features [0:5][0]
    
    top_5 = [features[0] for features in sorted_features[0:5]]
...
def answer_six():
    ...
    mean_train_score = [np.mean(scores) for scores in train_scores[0:6]]
    ...
    return mean_train_score, test_scores_mean
