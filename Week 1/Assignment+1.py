...
def answer_two():
    ...
    index_target = ['benign(1)','malignant(0)']
    
    class_distribution = dict(zip(index_target,series))
...
def answer_three():
    ...
    #cancerdf_s = np.split(cancerdf, cancerdf.iloc[:-1], axis=1)
    
    X = cancerdf.drop(labels='target', axis=1)
